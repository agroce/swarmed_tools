...
function tryTestDVG(code)
{
  ...
  try {
    ...
    if (e.message != "this.nnn is undefined") {
      printAndStop("Wrong error message: " + e, 101);
    }
  }
}
...
function checkForExtraParens(f, code)
{
  ...
  var ueuf = "" + euf;
  if (ueuf != uf)
     //printAndStop("Shouldn't the earlier round-trip test have caught this?", 106);
  ...
  for (i = 1; i < dps.length; ++i) {
    ...
    if (ueuf2 == ueuf) {
      ...
      print(ueuf);
      //printAndStop("Unexpected match!!!  Extra parens!?", 107);
    }
  }
}
...
function catNice(toks)
{
  ...
  for (i=0; i<toks.length; ++i) {
    if(typeof(toks[i]) != "string")
      printAndStop("Strange toks[i]: " + toks[i], 109);
